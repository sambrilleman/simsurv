...
also possible. If a user-defined [log] hazard function is specified, then this 
...
package documentation provides an example of this.
...
*development* version of the package. A stable version of the package will be 
available from CRAN once it is released.

## Getting Started

### Installation

You can install **simsurv** directly from GitHub using the **devtools** package. 
...
2. Generate times from a Gompertz model:

```{r example2}
s2 <- simsurv(dist = "gompertz", lambdas = 0.1, gammas = 0.05, x = covs)
```

3. Generate times from a 2-component mixture Weibull model:
...
4. Generate times from user-defined log hazard function:

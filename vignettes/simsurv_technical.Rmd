...
This vignette herein describes the methodology used to simulate event times in the **simsurv** package.
...
The two-component mixture distributions in **simsurv** are parameterised in the same way as the **survsim** Stata package. That is, they are additive on the survival scale, with a parameter defining the mixing proportions, i.e.
...
$$
  ...
  \left[
    ...
    { \pi \gamma_1 \lambda_1 (t^{\gamma_1-1}) \exp(-\lambda_1 (t^{\gamma_1})) + 
      (1-\pi) \gamma_1 \lambda_1 (t^{\gamma_1-1}) \exp(-\lambda_2 (t^{\gamma_2})) }

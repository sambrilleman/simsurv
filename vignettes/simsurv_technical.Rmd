...
Using the cumulative hazard inversion method one can easily simulate a survival time $T_i^s$ for individual $i$ by drawing a uniform random variable $U_i \sim U(0,1)$ and evaluating
...
where $H_0^{-1} [.]$ corresponds to the inverted cumulative baseline hazard, and $X_i$ is a vector of covariates with associated population-level (i.e. fixed effect) parameters $\beta$. Therefore, all that is required to generate the simulated event time is an independent draw of the random uniform variable $U_i$ and an invertible cumulative baseline hazard function. If an analytical solution to the inverse of the cumulative baseline hazard can be obtained, then the method is both simple and computationally efficient. The intuition behind this method is described next.
...
where $H_0(t)=\int_0^t h_0(s) ds$ is the cumulative baseline hazard evaluated at time $t$. Since the objective is to simulate new event times $T_i^s$ $(i=1,…,N)$ one needs to rearrange equation $X$ to solve for the event time; the equation resulting from the rearrangement was shown previously in equation $\ref{eq:bender_formula}$.
